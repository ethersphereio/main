...
contract MintableToken is ERC20, SafeMath, DAOControlled{
	...
	function resetFactories() onlyOwner{
	    numFactories = 0;
	}
	...
	function removeMintingFactory(address _factory) onlyOwner{
...
contract SphereToken is BurnableToken, Haltable {
  ...
  mapping (address => mapping (address => uint)) allowed;
    ...
    function SphereToken(string _name, uint8 _decimals,string  _symbol, bool _isTransferEnabled){
        require(_decimals == 4);
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
  ...
  modifier onlyPayloadSize(uint size) {
    ...
    function setTransferEnable(bool enabled) onlyOwner{
