...
contract PricingMechanism is Haltable, SafeMath{
    ...
    function setPricing() onlyOwner{
        ...
        priceList.push(PriceTier(uint(safeDiv(1 ether, 100 * factor)),0,1000 ether));
        priceList.push(PriceTier(uint((1 ether - (10 wei * factor)) / (90 * factor)),0,2000 ether));
        priceList.push(PriceTier(uint(1 ether / (80* factor)),0,2000 ether));
        ...
        priceList.push(PriceTier(uint((1 ether - (10 wei * factor))/ (15* factor)),0,50000 ether));
        numTiers = 9;
    }
...
contract CrowdSale is PricingMechanism, DAOController{
    ...
    function CrowdSale(address _tokenFactory, uint _decimals, uint _hardCapAmount){
        tokenFactory = SphereTokenFactory(_tokenFactory);
        require(_decimals == 4);
        decimals = _decimals;
        setPricing();
    ...
    function finalize() payable onlyOwner notFinalized afterFinalizeSet{
    ...
    function emergencyCease() payable onlyStarted onlyInEmergency onlyOwner {
        isFinalized = true;
        isStarted = false;
        multiSig.call.gas(150000).value(this.balance)();
    }
